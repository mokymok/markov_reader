...
class cmds_settings(commands.Cog, name="Settings"):
    ...
    async def settings(self, ctx, *, help=None):
        ...
        message1 = cleandoc(f"""
            ...
            :small_orange_diamond: Ignore Users: `{user_ignore}`
    ...
    @set.command(aliases=["user_ignore", "ignore_users", "ignoreusers"])
    @commands.has_permissions(administrator=True)
    async def userignore(self, ctx, value: bool):
        "Messages sent by users other than Markov are not read"
        await self.bot.settings.set(ctx.guild, "user_ignore", value)
        await ctx.send(f"Ignoring Users is now: {to_enabled[value]}")
